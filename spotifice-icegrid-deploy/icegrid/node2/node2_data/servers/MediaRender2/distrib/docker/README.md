...
Despliegue de Spotifice utilizando Docker con 2 nodos IceGrid independientes, simulando máquinas virtuales separadas. **Utiliza IcePatch2 para la distribución automática de archivos de la aplicación.**
...
│  │     │     │     │                                            │
│  │ IP2 │     │     │   IcePatch2 Server (distribución)         │
...
## IcePatch2 - Distribución de Archivos

Este despliegue utiliza **IcePatch2** para distribuir automáticamente los archivos de la aplicación a los nodos:

- **IcePatch2 Server**: Se ejecuta en el Nodo 1 y sirve los archivos de la aplicación
- **Distribución automática**: Los servidores MediaServer y MediaRender descargan automáticamente sus archivos desde IcePatch2 antes de iniciar
- **Directorio de trabajo**: Los servidores usan `${server.distrib}` como directorio de trabajo, que contiene los archivos distribuidos

### Funcionamiento

1. Al iniciar, se ejecuta `icepatch2calc` para generar checksums de los archivos
2. El servidor IcePatch2 se inicia en el Nodo 1
3. Cuando un servidor (MediaServer/MediaRender) se activa, IceGrid automáticamente:
   - Contacta al servidor IcePatch2
   - Descarga los archivos necesarios al directorio `distrib` del servidor
   - Inicia el servidor en ese directorio
...
Los archivos de la aplicación se distribuyen mediante **IcePatch2**.
